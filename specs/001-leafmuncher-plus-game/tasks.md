...
- [x] T070 [P] [US6] Module `theme`: bảng `Theme` `const` FOREST/DESERT (màu) + `theme_get`/`theme_count`/`theme_next` trong `Core/Src/theme.c` + `Core/Inc/theme.h` (hợp đồng [contracts/theme.md](contracts/theme.md)) — host test `test_theme.c`; `obstacle_sprite=NULL` (sprite 16×16 gộp T091); thêm `theme.c` vào `C_SOURCES`
- [x] T071 [P] [US5] Module `store`: `PersistData` + `store_init`/`store_get`/`store_set_*`/`store_commit`; ghi **sector 4 single-bank** (erase+program) qua `HAL_FLASH_*`, magic/version/crc, fallback mặc định khi trống/hỏng trong `Core/Src/store.c` + `Core/Inc/store.h`; codec thuần `store_codec.c` (CRC32/validate/defaults) → host test `test_store.c` (hợp đồng [contracts/store.md](contracts/store.md))
...
- [x] T073 [US6] `render` đọc `theme_get(gs->theme_id)`: nền sân/HUD/chữ/sâu/lá/chướng ngại theo `Theme` (thay màu hardcode) trong `Core/Src/render.c` — power-up giữ màu theo loại (semantic); menu/banner giữ palette riêng
- [x] T074 [US5] Render HUD chế độ Vô tận: nhãn "ENDLESS" thay "LV n" + "BEST n" (điểm cao từ `store` qua `render_set_endless_best`) trong `Core/Src/render.c`
- [x] T075 [US5/US6] MENU đa mục: START=Màn / ENDLESS=Vô tận / THEME=cuộn `theme_id` (FOREST↔DESERT), hiện tên theme; trong `Core/Src/game.c` + render — host test T075
- [x] T076 [US5/US6] Tích hợp `store` trong `Core/Src/apptasks.c` (GameTask): `store_init` lúc boot → nạp `theme_id` cho render + `render_set_endless_best`; sườn MENU→PLAYING nếu theme đổi → `store_set_theme`+`store_commit`; sườn →GAME_OVER chế độ Vô tận nếu `score`>`endless_high` → `store_set_endless_high`+`store_commit`+cập nhật HUD (phụ thuộc T070, T071, T075)
- [x] T077 Thêm `store.c`, `store_codec.c` vào `C_SOURCES` (Makefile) + **dành riêng sector 4 single-bank (`0x08010000`, 64 KB) cho store** trong `STM32F429XX_FLASH.ld` (vùng `STORE` riêng + thu `FLASH` code về 64 K → tràn báo lỗi link); ghi chú thêm lại sau mỗi CubeMX Generate; `./build.sh` 0 error (code 56.9 KB < 64 K). *(theme.c đã thêm ở T070.)*
- [x] T078 Demo M8: menu chọn mode + theme → chơi Vô tận lập điểm cao → tắt/bật nguồn giữ điểm cao & theme; `make -C test` xanh (5/5) + on-board ✓ (Acceptance US5/US6, FR-022..027). *(Save/resume ván US7 — T079–T084 — là Đợt B, chưa làm.)*
