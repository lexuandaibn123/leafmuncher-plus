 ...
 * Panel ILI9341: 240 (W) × 320 (H), RGB565.
 * 1 framebuffer = 240×320×2 = 153600 byte.
 * SDRAM (8MB @ 0xD0000000) dùng làm double-buffer. */
#define PANEL_W   240u
#define PANEL_H   320u
#define FB_BYTES  (PANEL_W * PANEL_H * 2u)
#define FB0_ADDR  0xD0000000u
#define FB1_ADDR  (FB0_ADDR + FB_BYTES)

static uint32_t g_fb[2]   = { FB0_ADDR, FB1_ADDR };
static uint8_t  g_back    = 1;

/* Hoán đổi buffer đồng bộ VSYNC qua ngắt line LTDC (tránh xé hình). */
#define LTDC_SWAP_LINE  323u
static volatile uint32_t g_swap_addr = 0;
static volatile uint8_t  g_swap_req  = 0;

/* Khởi tạo SDRAM (IS42S16400J) */
...
/* Refresh count = (15.625µs × 36MHz) − 20 ≈ 542 */
...
/* Gửi lệnh khởi tạo màn hình ILI9341 qua SPI5 */
...
static void panel_init(void) {
  ...
  lcd_cmd_n(0xCF, (const uint8_t[]){0x00,0xC1,0x30}, 3);
  lcd_cmd_n(0xED, (const uint8_t[]){0x64,0x03,0x12,0x81}, 4);
  lcd_cmd_n(0xE8, (const uint8_t[]){0x85,0x00,0x78}, 3);
  lcd_cmd_n(0xCB, (const uint8_t[]){0x39,0x2C,0x00,0x34,0x02}, 5);
  lcd_cmd_n(0xF7, (const uint8_t[]){0x20}, 1);
  lcd_cmd_n(0xEA, (const uint8_t[]){0x00,0x00}, 2);
  lcd_cmd_n(0xB1, (const uint8_t[]){0x00,0x1B}, 2);
  lcd_cmd_n(0xB6, (const uint8_t[]){0x0A,0xA2}, 2);
  lcd_cmd_n(0xC0, (const uint8_t[]){0x10}, 1);
  lcd_cmd_n(0xC1, (const uint8_t[]){0x10}, 1);
  lcd_cmd_n(0xC5, (const uint8_t[]){0x45,0x15}, 2);
  lcd_cmd_n(0xC7, (const uint8_t[]){0x90}, 1);
  lcd_cmd_n(0x36, (const uint8_t[]){0xC8}, 1);
  lcd_cmd_n(0xF2, (const uint8_t[]){0x00}, 1);
  lcd_cmd_n(0xB0, (const uint8_t[]){0xC2}, 1);  /* Đưa panel vào chế độ RGB */
  lcd_cmd_n(0xB6, (const uint8_t[]){0x0A,0xA7,0x27,0x04}, 4);
  lcd_cmd_n(0x2A, (const uint8_t[]){0x00,0x00,0x00,0xEF}, 4);
  lcd_cmd_n(0x2B, (const uint8_t[]){0x00,0x00,0x01,0x3F}, 4);
  lcd_cmd_n(0xF6, (const uint8_t[]){0x01,0x00,0x06}, 3);
  lcd_cmd(0x2C);
  lcd_cmd_n(0x26, (const uint8_t[]){0x01}, 1);
  lcd_cmd_n(0xE0, (const uint8_t[]){0x0F,0x29,0x24,0x0C,0x0E,0x09,0x4E,0x78,
                                    0x3C,0x09,0x13,0x05,0x17,0x11,0x00}, 15);
  lcd_cmd_n(0xE1, (const uint8_t[]){0x00,0x16,0x1B,0x04,0x11,0x07,0x31,0x33,
                                    0x42,0x05,0x0C,0x0A,0x28,0x2F,0x0F}, 15);
...
/* Thanh ghi màu OCOLR của DMA2D phải ở định dạng ARGB8888 */
...
/* Dùng DMA2D để tô một khối màu nhanh */
...
/* Chỉnh lại clock LTDC xuống ~6.25MHz cho panel ILI9341 */
...
void gfx_present(void) {
  ...
  uint32_t guard = 0;
  while (g_swap_req && ++guard < 2000000u) { }

  g_back ^= 1u;
}

/* Vẽ pixel có hỗ trợ xoay màn hình ngang/dọc */
